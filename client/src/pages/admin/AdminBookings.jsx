...
const AdminBookings = () => {
  ...
  const fetchBookings = useCallback(async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'โหลดข้อมูลการจองล้มเหลว'); 
    } finally { 
      setLoading(false); 
    }
  }, [filter]);
  ...
  const handleAddMonk = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'เพิ่มล้มเหลว');
    }
  };

  const handleDeleteMonk = async (id) => {
    if (!window.confirm("⚠️ ต้องการลบรายชื่อพระสงฆ์รูปนี้ออกจากระบบ?")) return;
    try {
      await monkAPI.delete(id);
      toast.success('ลบเรียบร้อย');
      fetchMonks();
    } catch (error) {
      toast.error('ไม่สามารถลบได้ เนื่องจากข้อมูลถูกใช้งานอยู่');
    }
  };
      ...
      try {
        ...
        if (Array.isArray(resData)) {
          ...
          if (hasStatusFlag) {
            ...
            const availableIds = resData.map(monk => String(monk.id ?? monk.monk_id ?? monk.monkId));
            const busyIds = allMonks
              .filter(monk => !availableIds.includes(String(monk.id ?? monk.monk_id ?? monk.monkId)))
              .map(monk => String(monk.id ?? monk.monk_id ?? monk.monkId));
    ...
    setSelectedMonkIds(prev => {
      if (prev.includes(monkIdStr)) {
        ...
        if (prev.length >= requiredMonks) {
          toast.warning(`สล็อตเต็มที่ ${requiredMonks} รูปแล้ว`);
          return prev;
        }
  ...
  const handleUpdateStatus = async (id, status) => {
    ...
    try {
      if (status === 'approved' && selectedMonkIds.length !== requiredMonks) {
        return toast.warning(`กรุณาจัดรายชื่อพระให้ครบตามจำนวนนิมนต์ (${requiredMonks} รูป)`);
      }
      ...
      toast.success(`อัปเดตสถานะคิวการจองเรียบร้อย`);
      setShowModal(false); 
      fetchBookings();
    } catch (error) { 
      toast.error(error.response?.data?.message || 'การอัปเดตล้มเหลว'); 
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try { 
      ...
      fetchBookings(); 
    } catch (error) { toast.error(error.response?.data?.message || 'ลบไม่สำเร็จ'); }
  };
  ...
  const handleAddType = async () => {
    ...
    try {
      ...
      fetchTypes();
    } catch (error) { toast.error(error.response?.data?.message || 'เพิ่มไม่สำเร็จ'); }
  };
  ...
  const handleDeleteType = async (id) => {
    ...
    try { 
      ...
      fetchTypes(); 
    } catch (error) { toast.error('ไม่สามารถลบได้เนื่องจากมีการใช้งานอยู่ในระบบ'); }
  };
      ...
      <div className="bg-slate-900 pt-16 pb-24 px-6 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          ...
          <div className="flex flex-wrap gap-3 w-full md:w-auto">
            <button onClick={() => setShowMonkModal(true)} className="flex-1 md:flex-initial flex items-center justify-center gap-2 px-6 py-4 bg-orange-500 hover:bg-orange-600 text-white rounded-2xl font-bold transition-all active:scale-95 shadow-lg shadow-orange-500/20">
              <Plus size={20} /> จัดการรายชื่อพระสงฆ์
            </button>
            <button onClick={() => setShowTypeModal(true)} className="flex-1 md:flex-initial flex items-center justify-center gap-2 px-6 py-4 bg-white/10 hover:bg-white text-white hover:text-slate-900 rounded-2xl font-bold transition-all border border-white/20 active:scale-95">
              <Settings size={20} /> ตั้งค่าประเภทพิธี
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 -mt-12 relative z-20">
        ...
        <div className="bg-white rounded-4xl shadow-xl shadow-slate-200/50 p-4 mb-10 flex flex-col lg:flex-row gap-4 items-center border border-slate-100">
          <div className="relative w-full lg:flex-1">
            ...
            <input type="text" placeholder="ค้นหาชื่อผู้จอง หรือพิธี..." className="w-full pl-14 pr-6 py-4 bg-slate-50 border-none rounded-2xl outline-none focus:ring-2 focus:ring-orange-500/20 transition-all font-medium" onChange={(e) => setSearchTerm(e.target.value)} />
          </div>
          <div className="flex bg-slate-100 p-1.5 rounded-2xl w-full lg:w-auto overflow-x-auto">
            {[
              ...
              <button key={t.id} onClick={() => setFilter(t.id)} className={`px-6 py-3 rounded-xl text-sm font-black transition-all whitespace-nowrap ${filter === t.id ? t.color : 'text-slate-400 hover:text-slate-600'}`}>{t.label}</button>
            ))}
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredBookings.map((booking) => {
              ...
              return (
                <motion.div layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} key={booking.id} className="group bg-white rounded-[40px] p-8 border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-orange-900/5 transition-all relative overflow-hidden">
                  <div className={`absolute top-0 left-0 w-full h-2 ${booking.status === 'approved' ? 'bg-emerald-500' : booking.status === 'rejected' ? 'bg-red-500' : 'bg-amber-500'}`} />
                  <div className="flex justify-between items-start mb-6">
                    <div className="w-14 h-14 bg-slate-50 group-hover:bg-orange-50 transition-colors rounded-2xl flex items-center justify-center text-slate-400 group-hover:text-orange-500"><User size={28} /></div>
                    <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${booking.status === 'approved' ? 'bg-emerald-50 text-emerald-600' : booking.status === 'rejected' ? 'bg-red-50 text-red-600' : 'bg-amber-50 text-amber-600'}`}>{booking.status === 'pending' ? 'รอดำเนินการ' : booking.status}</span>
                  </div>
                  <div className="space-y-4">
                    <div>
                      <h4 className="text-xl font-black text-slate-800 line-clamp-1">{booking.full_name}</h4>
                      <p className="text-slate-400 text-sm font-medium flex items-center gap-1.5 mt-1"><Phone size={14} /> {booking.phone || booking.phone_number || 'ไม่ระบุเบอร์โทร'}</p>
                    </div>
                    <div className="grid grid-cols-1 gap-2">
                      <div className="flex items-center gap-3 px-4 py-3 bg-slate-50 rounded-2xl text-sm font-bold text-slate-600"><Calendar size={16} className="text-orange-500" />{booking.booking_date ? new Date(booking.booking_date).toLocaleDateString('th-TH', { day: '2-digit', month: 'short', year: '2-digit' }) : '-'}</div>
                      <div className="flex items-center gap-3 px-4 py-3 bg-slate-50 rounded-2xl text-sm font-bold text-slate-600">{timeInfo.icon}{timeInfo.label} {booking.booking_time ? `(${booking.booking_time.substring(0, 5)} น.)` : ''}</div>
                      <div className="flex items-center gap-3 px-4 py-3 bg-orange-50/50 border border-orange-100 rounded-2xl text-sm font-bold text-orange-600"><Info size={16} />{booking.booking_type_name || 'ไม่ระบุประเภทพิธี'}</div>
                    </div>
                    <div className="flex gap-2 pt-2">
                      <button onClick={() => openBookingDetail(booking)} className="flex-1 py-4 bg-slate-900 text-white rounded-2xl font-bold text-sm hover:bg-slate-800 transition-all flex items-center justify-center gap-2">จัดการคำขอ <ChevronRight size={16} /></button>
                      <button onClick={() => handleDelete(booking.id)} className="p-4 bg-red-50 text-red-500 rounded-2xl hover:bg-red-500 hover:text-white transition-all"><Trash2 size={20} /></button>
                    </div>
                  </div>
                </motion.div>
              );
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-[48px] w-full max-w-2xl shadow-2xl relative z-10 overflow-hidden max-h-[90vh] flex flex-col">
              ...
              <div className="p-8 overflow-y-auto space-y-6 no-scrollbar">
                ...
                <div className="p-8 border border-slate-100 rounded-4xl space-y-4">
                  <div className="flex justify-between items-center"><span className="text-slate-400 font-bold text-xs uppercase">พิธีการ</span><span className="font-black text-orange-600 text-lg">{selectedBooking.booking_type_name}</span></div>
                  <div className="flex justify-between items-center border-t border-slate-50 pt-4"><span className="text-slate-400 font-bold text-xs uppercase">จำนวนพระ</span><span className="font-black text-slate-800 text-lg">{(selectedBooking.monks_count ?? selectedBooking.monksCount ?? 0)} รูป</span></div>
                ...
                <div className="space-y-3 pt-2">
                  ...
                  <textarea className="w-full p-6 bg-slate-50 rounded-4xl border-none outline-none focus:ring-2 focus:ring-orange-500/20 h-32 resize-none transition-all font-medium" placeholder="พิมพ์ข้อความตอบกลับโยมผู้จองที่นี่..." value={adminResponse} onChange={(e) => setAdminResponse(e.target.value)} />
                </div>
              </div>
              ...
              <div className="p-8 bg-slate-50 flex flex-col sm:flex-row gap-3">
                {selectedBooking.status === 'pending' ? (
                  <>
                    <button onClick={() => handleUpdateStatus(selectedBooking.id, 'approved')} className="flex-1 py-5 bg-emerald-500 text-white rounded-2xl font-black text-lg shadow-xl shadow-emerald-500/20 hover:bg-emerald-600 transition-all active:scale-95 flex items-center justify-center gap-2"><Check size={24} /> อนุมัติการจอง</button>
                    <button onClick={() => handleUpdateStatus(selectedBooking.id, 'rejected')} className="flex-1 py-5 bg-red-50 text-red-500 rounded-2xl font-black text-lg hover:bg-red-100 transition-all"><X size={24} className="inline mr-2" /> ปฏิเสธ</button>
                  </>
                ) : (
                  <button onClick={() => handleUpdateStatus(selectedBooking.id, 'pending')} className="w-full py-5 bg-white border border-slate-200 text-slate-600 rounded-2xl font-black text-lg flex items-center justify-center gap-2 hover:bg-slate-100 transition-all"><RotateCcw size={24} /> ย้ายกลับไปรอดำเนินการ</button>
                )}
              </div>
            </motion.div>
          </div>
        ...
        {showMonkModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 50, opacity: 0 }} className="bg-white rounded-[48px] w-full max-w-2xl shadow-2xl relative z-10 overflow-hidden max-h-[90vh] flex flex-col">
              <div className="p-8 border-b border-slate-50 flex justify-between items-center"><h3 className="text-2xl font-black text-slate-800 flex items-center gap-2"><Users className="text-orange-500" size={26} /> จัดการรายชื่อพระสงฆ์</h3><button onClick={() => setShowMonkModal(false)} className="p-3 hover:bg-slate-50 rounded-full transition-colors"><X size={28} /></button></div>
              <div className="p-8 overflow-y-auto space-y-8 no-scrollbar">
                <div className="bg-orange-50 p-8 rounded-[40px] border border-orange-100">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    <input placeholder="ระบุชื่อพระสงฆ์ (เช่น พระสมชาย ชินวํโส)" className="flex-1 px-6 py-4 rounded-2xl bg-white border-none outline-none focus:ring-2 focus:ring-orange-500/20 font-bold shadow-sm" value={newMonkName} onChange={(e) => setNewMonkName(e.target.value)} />
                    <button onClick={handleAddMonk} className="py-4 px-8 bg-orange-500 text-white rounded-2xl font-black shadow-lg shadow-orange-500/30 hover:bg-orange-600 transition-all flex items-center justify-center gap-2 whitespace-nowrap"><Plus size={20} /> เพิ่มรายชื่อ</button>
                  </div>
                </div>
                <div className="space-y-4">
                  <h4 className="text-slate-400 font-black text-xs uppercase tracking-widest ml-2">พระสงฆ์ทั้งหมดในระบบ</h4>
                  {allMonks.length === 0 ? <p className="text-center py-4 text-slate-300 italic">ไม่มีข้อมูลรายชื่อพระสงฆ์</p> : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {allMonks.map(monk => (
                        <div key={monk.id} className="p-5 rounded-3xl bg-slate-50 border border-slate-100 flex justify-between items-center group">
                          ...
                          <button onClick={() => handleDeleteMonk(monk.id)} className="w-10 h-10 shrink-0 flex items-center justify-center bg-white text-red-400 rounded-xl hover:bg-red-500 hover:text-white transition-all shadow-sm border border-slate-100"><Trash2 size={16} /></button>
                        </div>
                      ))}
                    </div>
                  )}
      ...
      <AnimatePresence>
        {showTypeModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 50, opacity: 0 }} className="bg-white rounded-[48px] w-full max-w-2xl shadow-2xl relative z-10 overflow-hidden max-h-[90vh] flex flex-col">
              <div className="p-8 border-b border-slate-50 flex justify-between items-center"><h3 className="text-2xl font-black text-slate-800">ตั้งค่าประเภทพิธี</h3><button onClick={() => setShowTypeModal(false)} className="p-3 hover:bg-slate-50 rounded-full transition-colors"><X size={28} /></button></div>
              <div className="p-8 overflow-y-auto space-y-8 no-scrollbar">
                <div className="bg-orange-50 p-8 rounded-[40px] border border-orange-100">
                  <h4 className="text-orange-600 font-black text-xs uppercase tracking-widest mb-6">เพิ่มประเภทพิธี</h4>
                  <div className="space-y-4">
                    <input placeholder="ชื่อพิธี (เช่น ขึ้นบ้านใหม่)" className="w-full px-6 py-4 rounded-2xl bg-white border-none outline-none focus:ring-2 focus:ring-orange-500/20 font-bold shadow-sm" value={newType.name} onChange={(e) => setNewType({...newType, name: e.target.value})} />
                    <div className="relative">
                      <input type="number" placeholder="ระยะเวลา" className="w-full px-6 py-4 rounded-2xl bg-white border-none outline-none focus:ring-2 focus:ring-orange-500/20 font-bold shadow-sm" value={newType.duration} onChange={(e) => setNewType({...newType, duration: e.target.value})} />
                    ...
                    <textarea placeholder="คำอธิบาย..." className="w-full px-6 py-4 rounded-2xl bg-white border-none outline-none focus:ring-2 focus:ring-orange-500/20 font-medium shadow-sm h-24 resize-none" value={newType.description} onChange={(e) => setNewType({...newType, description: e.target.value})} />
                    <button onClick={handleAddType} className="w-full py-4 bg-orange-500 text-white rounded-2xl font-black shadow-lg shadow-orange-500/30 hover:bg-orange-600 transition-all">เพิ่มประเภทพิธี</button>
                  </div>
                </div>
                <div className="space-y-4">
                  <h4 className="text-slate-400 font-black text-xs uppercase tracking-widest ml-2">รายการปัจจุบัน</h4>
                  {bookingTypes.length === 0 ? <p className="text-center py-4 text-slate-300 italic">ไม่มีข้อมูลประเภทพิธี</p> : bookingTypes.map(t => (
                    <div key={t.id} className="p-5 rounded-3xl bg-slate-50 border border-slate-100 flex justify-between items-center group">
                      <div className="flex-1"><h5 className="font-bold text-slate-800 text-base">{t.name}</h5><p className="text-xs font-medium text-slate-500 mt-1 flex items-center gap-1"><Clock size={12}/> {t.duration_minutes || 60} นาที</p></div>
                      <button onClick={() => handleDeleteType(t.id)} className="w-10 h-10 shrink-0 flex items-center justify-center bg-white text-red-400 rounded-xl hover:bg-red-500 hover:text-white transition-all shadow-sm border border-slate-100 ml-4"><Trash2 size={16} /></button>
                    </div>
                  ))}
                </div>
              </div>
